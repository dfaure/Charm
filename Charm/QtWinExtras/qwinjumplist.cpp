...
static QString createArguments(const QStringList &arguments)
{
    ...
    for (int i=0; i<arguments.size(); ++i) {
        ...
        const static QRegularExpression quotesRE(QLatin1String("(\\\\*)\""));
        tmp.replace(quotesRE, QLatin1String("\\1\\1\\\""));
        if (tmp.isEmpty() || tmp.contains(u' ') || tmp.contains(u'\t')) {
            // The argument must not end with a \ since this would be interpreted
            // as escaping the quote -- rather put the \ behind the quote: e.g.
            // rather use "foo"\ than "foo\"
            int j=tmp.length();
            while (j > 0 && tmp.at(j - 1) == u'\\')
                --j;
            tmp.insert(j, u'"');
            tmp.prepend(u'"');
        }
...
QWinJumpListItem *QWinJumpListPrivate::fromIShellLink(IShellLinkW *link)
{
    ...
    link->GetPath(buffer, buffersize-1, nullptr, 0);
